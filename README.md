...
- 과제의 목적인 cli 환경 구현 동시에 대화형 기능 UI 까지 접목함
- 환경설정을 제외한 나머지 메뉴는 접근 시 cli 와 대화형 모두 사용할 수 있음.

---

### 전문가 분들을 위한 테스트 가이드

- 수월한 테스트를 위해 가이드 페이지를 따로 분리했습니다.
- 테스트 수행 시 링크된 페이지로 이동해서 수행해주세요. [테스트 가이드 활용](./documentation/result/12.테스트가이드-전문가.md)
...
───────────────────────────────────────────────────
  ...
  4. AI 감성 분석 (sentiment)
───────────────────────────────────────────────────
  5. 뉴스 목록 조회 (list)
───────────────────────────────────────────────────
  6. AI 종합 인사이트 분석 (analyze)
...
- **PM (박순몽)**: 프로젝트 운영, 일정 관리(WBS/간트차트), 전체 스켈레톤 코드 작성 및 라우팅/설계, 로그 기록 설계/개발, 프로그램 보강 [cite: 1]
- **개발자 1, 2 (김병국, 이원일)**: 수집, 정제, AI 연동, 시각화, 내보내기 등 모듈별 기능 분담 개발[cite: 1]
- **QA (김정진)**: 단위/통합 테스트 시트 작성, 예외 상황 검증 및 크래시 테스트 피드백[cite: 1]
...
4. ** 크롤링 윤리 정책 준수를 위한 처리**:
   - **해결**: 5가지 윤리정책 준수 지침 마련 및 수행 [윤리정책](./documentation/result/9.크롤링윤리정책준수.md)]
...
7. [**정기 실행 스케줄링 가이드**](./documentation/result/6.정기실행_스케줄링_가이드.md)
8. [**로그 수준 정의**](./documentation/result/8.로그수준정의.md)
9. [**데이터 수집 방식 API/RSS vs 웹 크롤링) 비교 및 품질 보고서**](./documentation/result/10.수집방식_비교_품질분석.md)
